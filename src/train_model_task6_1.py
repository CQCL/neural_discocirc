...
from big_network_models.is_in_one_network_task6 import \
    IsInOneNetworkTrainerTask6
...
from utils.callbacks import ValidationAccuracy, TrainAccuracy, \
...
def train(base_path, save_path, vocab_path,
    ...
    # train_callback = TrainAccuracy(discocirc_trainer.get_accuracy,
    #                                          interval=1,
    #                                          log_wandb=config["log_wandb"])

    print('training...')

    callbacks = [tb_callback, validation_callback] #, train_callback] # checkpoint_callback]
...
if config["log_wandb"]:
    wandb.init(project="initial_tests", name="is_in_task6_debug_100", entity="sarajones", config=config)
